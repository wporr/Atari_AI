...
#				self.env.render()
				...
				if initialize_net:
					action, qVals = self.Predict(t, preObs, initialize_net)
					initialize_net = False
				else:
					action, qVals = self.Predict(t, preObs)
	...
	def Predict(self, t, X, initialize_net=False):
		print(X)
		if initialize_net == False:
			...
			qVals = self.net.predict(
		                input_fn=input_fn)
		elif initialize_net == True:
         		qVals = self.train_net(initialize_net=True, preObs=X)
	...
	def train_net(self, preObs, initialize_net=False):

		 #The first forward pass of a net must be made by Estimator().train()
		if initialize_net:
			input_function = tf.estimator.inputs.numpy_input_fn(
					x = {'X':np.asarray([preObs])},
					batch_size = 1,
					shuffle = False)
			qVals = self.net.train(
...
#			training = mode == tf.estimator.ModeKeys.TRAIN)
		...
		if mode == tf.estimator.ModeKeys.TRAIN:
			...
			if len(features) == 1: #meaning we're in initialization mode
				return tf.estimator.EstimatorSpec(
					mode = mode,
					predictions = prediction)
			else:
